...
import { UploadCloudIcon, CheckCircleIcon, AlertTriangleIcon, LightbulbIcon } from './components/icons';
...
const ExamView: React.FC<ExamViewProps> = ({ questionSet, attempt, setAttempt, currentQuestionIndex, setCurrentQuestionIndex, onSubmit, onExit, onGetHint, hint, isHintLoading }) => {
    ...
    const { remainingTime, start, setTime } = useTimer(attempt.timer.remainingSec, onSubmit);
        ...
        setAttempt(prev => {
            ...
            if (answer) { // has answer
                if(currentStatus === QuestionStatus.MarkedForReview || currentStatus === QuestionStatus.AnsweredAndMarked) {
                    newStatus = QuestionStatus.AnsweredAndMarked;
                } else {
                    newStatus = QuestionStatus.Answered;
                }
            } else { // no answer
                if(currentStatus === QuestionStatus.AnsweredAndMarked) {
                    newStatus = QuestionStatus.MarkedForReview;
            ...
            return { ...prev, answers: updatedAnswers, statuses: updatedStatuses };
        });
    };
    ...
    const handleMarkForReview = () => {
        ...
        if (currentStatus === QuestionStatus.MarkedForReview) {
            newStatus = attempt.answers[qid] ? QuestionStatus.Answered : QuestionStatus.NotAnswered;
        } else if (currentStatus === QuestionStatus.AnsweredAndMarked) {
             newStatus = QuestionStatus.Answered;
        } else if (attempt.answers[qid]) {
            newStatus = QuestionStatus.AnsweredAndMarked;
        } else {
            newStatus = QuestionStatus.MarkedForReview;
        }
    ...
    const handleQuestionSelect = (index: number) => {
        if (currentQuestion) {
            ...
            if (currentStatus === QuestionStatus.NotVisited) {
                updateStatus(currentQuestion.qid, QuestionStatus.NotAnswered);
            }
        }
        
        const newQid = questionSet.questions[index].qid;
        const newStatus = attempt.statuses[newQid];
        if (newStatus === QuestionStatus.NotVisited) {
            updateStatus(newQid, QuestionStatus.NotAnswered);
        }
    ...
    const clearResponse = () => {
        if (!currentQuestion) return;
        handleAnswerChange(currentQuestion.qid, "");
    };

    if (!currentQuestion) {
        return (
            <div className="flex items-center justify-center h-screen">
                <div className="text-center p-8 bg-surface-dark rounded-lg shadow-xl">
                    <AlertTriangleIcon className="w-12 h-12 mx-auto text-red-500 mb-4" />
                    <h2 className="text-2xl font-semibold text-white">No Questions Found</h2>
                    <p className="text-text-secondary-dark mt-2 max-w-sm">
                        The AI parser could not detect any questions. This can happen with very low-quality scans or non-standard document formats.
                    </p>
                    <button
                        onClick={onExit}
                        className="mt-6 px-5 py-2.5 bg-brand-primary hover:bg-brand-secondary rounded-md font-semibold transition-colors"
                    >
                       Try Another File
                    </button>
                </div>
        ...
        <div className="flex flex-col h-screen">
            <header className="flex items-center justify-between p-3 bg-surface-dark border-b border-gray-700 shadow-md sticky top-0 z-20">
                ...
                <div className="flex items-center space-x-4">
                    ...
                    <button onClick={onSubmit} className="px-4 py-2 bg-green-600 hover:bg-green-700 rounded-md font-semibold transition-colors">Submit Test</button>
                </div>
            </header>
                ...
                <main className="flex-1 flex flex-col overflow-y-auto">
                    <div className={`flex-1 grid ${currentPassage ? 'grid-cols-1 md:grid-cols-2' : 'grid-cols-1'} gap-4 p-4`}>
                        ...
                        <div className={`bg-surface-dark rounded-lg p-6 overflow-y-auto max-h-[calc(100vh-200px)] ${!currentPassage ? 'col-span-1' : ''}`}>
                             <div className="flex justify-between items-start mb-4">
                                <div>
                                    ...
                                    {currentQuestion.section && (
                                        <span className="text-xs font-medium bg-brand-primary/20 text-brand-accent px-2 py-1 rounded-full">{currentQuestion.section} {currentQuestion.topic && ` - ${currentQuestion.topic}`}</span>
                                    )}
                                </div>
                            ...
                            {showHint && (
                                <div className="bg-yellow-900/50 border border-yellow-700 text-yellow-200 px-4 py-3 rounded-lg mb-4 flex items-start space-x-3">
                                    <LightbulbIcon className="w-5 h-5 mt-1 flex-shrink-0" />
                                    <p>{hint.text}</p>
                                </div>
                            )}

                            {currentQuestion.figureRefs && currentQuestion.figureRefs.length > 0 && (
                                <div className="mb-4 space-y-2 rounded-lg p-2 border border-gray-700 bg-background-dark">
                                    {currentQuestion.figureRefs.map((fig, index) => (
                                    <img key={index} src={fig.dataUrl} alt={`Figure for question ${currentQuestion.qid}`} className="w-full h-auto rounded-md" />
                                    ))}
                                </div>
                            )}
                            ...
                            {currentQuestion.qtype === QuestionType.MCQ && (
                                <div className="space-y-3">
                                    {currentQuestion.options.map(option => (
                                        <label key={option.label} className={`flex items-start p-3 rounded-lg border-2 cursor-pointer transition-colors ${attempt.answers[currentQuestion.qid] === option.label ? 'bg-brand-secondary/30 border-brand-secondary' : 'border-gray-700 hover:border-gray-600'}`}>
                                            <input
                                                type="radio"
                                                name={`q_${currentQuestion.qid}`}
                                                value={option.label}
                                                checked={attempt.answers[currentQuestion.qid] === option.label}
                                                onChange={() => handleAnswerChange(currentQuestion.qid, option.label)}
                                                className="w-4 h-4 mt-1 text-brand-secondary bg-gray-700 border-gray-600 focus:ring-brand-secondary focus:ring-2"
                                            />
                                            <span className="ml-3 text-text-primary-dark whitespace-pre-wrap">{option.label}. {option.text}</span>
                                        </label>
                                    ))}
                                </div>
                            )}

                             {currentQuestion.qtype === QuestionType.TITA && (
                                <input
                                    type="text"
                                    value={attempt.answers[currentQuestion.qid] || ''}
                                    onChange={(e) => handleAnswerChange(currentQuestion.qid, e.target.value)}
                                    placeholder="Type your answer here"
                                    className="w-full bg-gray-900 border border-gray-700 rounded-md p-3 focus:ring-2 focus:ring-brand-secondary focus:border-brand-secondary outline-none"
                                />
                            )}
                        </div>
                    </div>
                    <div className="flex items-center justify-between p-3 border-t border-gray-700 sticky bottom-0 bg-surface-dark z-10">
                        <div className="flex space-x-2">
                             <button onClick={handleMarkForReview} className="px-4 py-2 bg-purple-600 hover:bg-purple-700 rounded-md font-semibold transition-colors">Mark for Review</button>
                             <button onClick={clearResponse} className="px-4 py-2 bg-gray-600 hover:bg-gray-700 rounded-md font-semibold transition-colors">Clear</button>
                        </div>
                         <div className="flex space-x-2">
                            <button onClick={prevQuestion} disabled={currentQuestionIndex === 0} className="px-4 py-2 bg-brand-primary hover:bg-brand-secondary rounded-md font-semibold transition-colors disabled:bg-gray-700 disabled:cursor-not-allowed">Previous</button>
                            <button onClick={nextQuestion} disabled={currentQuestionIndex === questionSet.questions.length - 1} className="px-4 py-2 bg-brand-primary hover:bg-brand-secondary rounded-md font-semibold transition-colors disabled:bg-gray-700 disabled:cursor-not-allowed">Next</button>
                        </div>
                    </div>
                </main>
                
                <aside className="w-80 bg-surface-dark border-l border-gray-700 overflow-y-auto p-4">
                    <QuestionNavigator
...
const ResultsView: React.FC<{ attempt: Attempt; questionSet: QuestionSet }> = ({ attempt, questionSet }) => {
    ...
    return (
        <div className="flex flex-col items-center justify-center min-h-screen p-4 sm:p-6">
            <div className="w-full max-w-3xl bg-surface-dark rounded-xl shadow-2xl p-6 sm:p-8">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-center">
                    <div className="bg-background-dark p-4 rounded-lg">
                        <p className="text-sm text-text-secondary-dark">Answered</p>
                        <p className="text-2xl font-bold">{score.totalAnswered} / {questionSet.meta.totalQuestions}</p>
                    </div>
                    <div className="bg-background-dark p-4 rounded-lg">
                        <p className="text-sm text-status-answered">MCQ Correct</p>
                        <p className="text-2xl font-bold">{score.mcqCorrect}</p>
                    </div>
                     <div className="bg-background-dark p-4 rounded-lg">
                        <p className="text-sm text-status-not-answered">MCQ Wrong</p>
                        <p className="text-2xl font-bold">{score.mcqWrong}</p>
                    </div>
                    <div className="bg-background-dark p-4 rounded-lg">
                        <p className="text-sm text-blue-400">TITA Correct</p>
                        <p className="text-2xl font-bold">{score.titaCorrect}</p>
                    </div>
                </div>
